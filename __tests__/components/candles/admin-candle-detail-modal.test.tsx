...
describe('AdminCandleDetailModal', () => {
  async function renderAndWait() {
    await act(async () => {
      render(
        <AdminCandleDetailModal candle={mockCandle} open={true} onOpenChange={() => {}} />
      );
    });
  ...
  it('renderiza correctamente con datos válidos (happy path)', async () => {
    await renderAndWait();
    expect(screen.getByText((content) => content.includes('Aroma Seleccionado'))).toBeInTheDocument();
  ...
  it('no renderiza nada si candle es null (not happy path)', () => {
    const { container } = render(
      <AdminCandleDetailModal candle={null} open={true} onOpenChange={() => {}} />
    );
    expect(container.firstChild).toBeNull();
  });

  it('muestra el QR si existe qrUrl', async () => {
    await renderAndWait();
    expect(screen.getByText((content) => content.includes('Código QR'))).toBeInTheDocument();
  });
